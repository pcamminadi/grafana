...
func TestIntegrationGitFiles_UpdateFolderMetadataOnNewBranch(t *testing.T) {
	...
	requireHTTPOK(t, resp, "creating folder on default branch")
	...
	t.Run("rename folder on new branch succeeds", func(t *testing.T) {
		...
		requireHTTPOK(t, resp, "PUT to rename folder on new branch")
	...
	t.Run("second rename on the same branch updates existing branch", func(t *testing.T) {
		...
		requireHTTPOK(t, resp, "first rename on new branch")
		...
		requireHTTPOK(t, resp, "second rename on existing branch")
	...
	t.Run("rename nested folder on new branch succeeds", func(t *testing.T) {
		...
		requireHTTPOK(t, resp, "creating nested folder on default branch")
		...
		requireHTTPOK(t, resp, "PUT to rename nested folder on new branch")
...
// requireHTTPOK reads and closes the response body, then asserts HTTP 200.
func requireHTTPOK(t *testing.T, resp *http.Response, context string) {
	t.Helper()
	body := drainAndClose(t, resp)
	require.Equal(t, http.StatusOK, resp.StatusCode, "%s should succeed: %s", context, string(body))
}

// drainAndClose reads the response body fully and closes it, returning the bytes.
